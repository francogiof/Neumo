...
export function updateCiudadanoProfile(userId: number, profile: Partial<{
  name: string;
  age: number;
  cv_path: string;
  linkedin: string;
  ...
  // Only update fields that exist in the ciudadano_table schema
  const allowed = [
  ...
  console.log('[updateCiudadanoProfile] Updating user', userId, 'with:', filtered);
  db.prepare(`UPDATE ciudadano_table SET ${setClause} WHERE user_id = ?`).run(...values, userId);
